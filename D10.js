...
*/
me.skills=["javascript","java","phyton","c",]
console.log(me)
...
const whoIBigger=function(n1,n2){
  return Math.max(n1,n2)
}
console.log(whoIBigger(5,5))
...
console.log(onlyLetters("simone 10 ciao 100"))
...
const isThisAnEmail=function(string){
  if(string.includes('@')&&string.charAt(0)!=='@'){
  return true
}else{
  return false
}
...
console.log(isThisAnEmail('simonegmail.com'))
...
*/
  ...
  function howManyDays(data) {
    ...
    const dateInMillis = Date.parse(data);
  ...
  const myDate = new Date(1995,10,4);
...
//risoluzione con ausilio google
...
*/
function searchByTitle(ricercastringa, movies) {
  const searchResults = movies.filter(movie => {
    const title = movie.Title.toLowerCase();
    return title.includes(ricercastringa.toLowerCase());
  });
...
*/
const searchAndDivide = (str, array = movies) => {
  const obj = {match:[],unmatch:[]}
  obj.match.push(array.filter(movie => movie.Title.includes(str)))
  obj.unmatch.push(array.filter(movie => !movie.Title.includes(str)))
  return obj
}
...
const removeIndex = function (n) {
  const remove = movies.filter((elemento) =>{
    return movies.indexOf(elemento) !== n
  })
  return remove
}
...
*/
const cicleF=function(){
  let all=document.querySelectorAll('td')
  all.forEach((td)=>{
    return console.log(td.innerText)
  })
}
...
*/
const addElementUl=function(){
  let liNuovo=document.createElement('li')
  let ul=document.getElementById('myList')
  ul.appendChild(liNuovo)
  return
}
...
const addClass=function(){
  let trElement=Array.from(document.getElementsByTagName('tr'))
  trElement.forEach((e)=>{
    e.classList.add('tr')
  })
}
...
const searchResults = searchByTitle('av', movies);
